...
function init (env, ctx) {
  ...
  function getAlarm (level, group) {
    ...
    if (!alarm) {
      ...
      alarms[key] = alarm;
      // Gate first emission for snoozeable alarms (WARN/URGENT) on this process
      // until storage refresh resolves OR the deadline passes. Closes the cold-start
      // race where a fresh process emits an alarm that another instance already snoozed.
      // INFO and lower are not snoozeable (see findUnSnoozeable) so do not gate them.
      var isSnoozeable = ctx.levels && level >= ctx.levels.WARN;
      if (isSnoozeable && ctx.alarmStorage && typeof ctx.alarmStorage.getSnooze === 'function') {
        ...
        ctx.alarmStorage.getSnooze(level, group).then(function (doc) {
          ...
          console.warn('alarmStorage.getSnooze failed for ' + level + '-' + group + ': ' + err.message);
          alarm.refreshPending = false;
        });
      }
    }
