...
function boot (env, language) {
  ...
  function setupInternals (ctx, next) {
    ...
    // Bind alarmStorage before notifications so getAlarm sees it on first call.
    // The TTL index creation itself happens in the later ensureIndexes step.
    ctx.alarmStorage = require('../storage/alarmStorage')(ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    // alarmStorage is bound earlier in setupInternals; this step only ensures
    // its TTL index exists.
    ctx.alarmStorage.ensureIndexes().catch(function (err) {
      console.error('alarmStorage.ensureIndexes FAILED for collection alarmsnooze' +
        ' — TTL index not created, expired snooze docs will accumulate' +
