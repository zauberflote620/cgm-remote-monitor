...
function config () {
  ...
  Object.keys(process.env).forEach((key) => {
    shadowEnv[key.trim()] = process.env[key].trim();
  });
...
function findExtendedSettings (envs) {
  ...
  env.settings.enable.forEach(function eachEnable (enable) {
    if (enable.trim()) {
      Object.entries(envs).forEach(([key, value]) => {
        var env = normalizeEnv(key);
        if (env.startsWith(enable.toUpperCase() + '_')) {
          var split = env.indexOf('_');
          if (split > -1 && split <= env.length) {
            var exts = extended[enable] || {};
            extended[enable] = exts;
            var ext = camelCase(env.substring(split + 1).toLowerCase());
